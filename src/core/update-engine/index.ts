/**
 * Manel Core — Update Engine
 *
 * Queries external APIs to find the latest version of development
 * technologies. Includes a TTL-based in-memory cache to avoid
 * redundant requests. No Electron or IPC dependencies.
 *
 * @module core/update-engine
 */
...
/**
 * Sort version entries by cycle (semver-like) in descending order.
 *
 * @param entries - Array of objects with a 'cycle' field
...
/**
 * Configuration for each technology's version source.
 * Maps technology name to URL + parser function.
 */
export const TECH_SOURCES: Record<string, SourceConfig> = {
  node: {
...
// ============================================================================
// 5. Singleton Cache Instance
// ============================================================================

/** Shared cache instance for all version queries */
const versionCache = new VersionCache()

/**
 * Get the shared cache instance (for testing/invalidation).
 *
 * @returns The shared VersionCache instance
 */
export function getCache(): VersionCache {
  return versionCache
}
...
/**
 * Get the latest version for a technology.
 *
...
/**
 * Get the latest versions for all known technologies.
 *
 * Queries all sources in parallel and returns a map of
 * technology name to latest version.
 *
 * @returns Map of technology name to latest version (null if unavailable)
 */
